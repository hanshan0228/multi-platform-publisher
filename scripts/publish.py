...
def publish_from_markdown(
    ...
    try:
        ...
        print(f"  API连接失败：{e}")
        print("  请检查 multi-platform-publisher.yaml 中的 app_id / app_secret 配置")
        sys.exit(1)
...
示例：
  ...
  python publish.py --input article.md --cover cover.jpg --ai-score-threshold 55
    ...
    parser.add_argument("--account", help="指定公众号账号（对应 multi-platform-publisher.yaml 中的账号名）")
    parser.add_argument(
        "--sync",
...
def _resolve_config(args):
    ...
    try:
        ...
        if args.sync_from_config:
            ...
            print("--sync-from-config 需要有效的 multi-platform-publisher.yaml 配置。", file=sys.stderr)
            sys.exit(1)
